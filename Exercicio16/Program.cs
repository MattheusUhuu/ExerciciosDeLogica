/*
16 - Escreva um programa para gerar uma matriz quadrada P de dimensão N.
     Imprimir os elementos da abaixo da diagonal principal (inclusa) e abaixo da diagonal secundária (inclusa).
...
public class MyArray
{
    public void PrintDiagonals(int[,] array)
    {
        ...
        PrintElementsBelowTheSecondaryDiagonal(array);
    }
    ...
    private void PrintElementsBelowTheSecondaryDiagonal(int[,] array)
    {
        WriteLine("---------------------------------------------------------------- \n Valores de teste Abaixo da diagonal Secundaria: 39, 92, 75, 95, 63");
        for (int rows = array.GetLowerBound(0) + 1; rows <= array.GetUpperBound(0); rows++)
        {
            for (int columns = array.GetUpperBound(1); columns > array.GetLowerBound(1); columns--)
            {
                WriteLine($"Linha: {rows}, Coluna: {columns}, Valor: {array[rows, columns]}");
                rows++;
            }
